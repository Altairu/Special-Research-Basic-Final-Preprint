...
\section{はじめに}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
近年，移動ロボット等の自律移動において，自己位置推定は基盤となる技術である．
LiDARやカメラを用いた手法は高精度な位置情報が得られる反面，計算コストが高く，更新レートが低い（数Hz〜数十Hz）という特性がある．
一方，MEMS-IMUはkHzオーダーの高周波計測が可能であるが，センサノイズの積分により位置誤差が時間の二乗で累積する課題がある．
...
そこで本研究では，数秒から10秒オーダーの遅延が存在する環境下において，MCUの限られた計算資源で実行可能な高精度補正手法「遅延バイアスフィードバック法」を提案する．
...
IMUから得られる加速度 $\boldsymbol{a}_{body}$ と角速度 $\boldsymbol{\omega}$ を用い，Global座標系での位置 $\boldsymbol{p}$ と速度 $\boldsymbol{v}$ を推定する．
...
遅延観測 $\boldsymbol{p}_{obs}$ が到着した際，以下の手順で補正を行う．また，Fig. \ref{fig:proposed_block} にブロック図を示す．
...
これは，ソナーの取得周期やSLAM処理に必要とされる時間を想定して設定した．

\subsection{比較手法}
以下の3手法の実装と性能比較を行った．
\begin{enumerate}
  \item \textbf{単純リセット :} 観測位置のみを修正する手法．
  \item \textbf{Augmented KF:} 加速度バイアスを状態に含む状態拡張型線形カルマンフィルタ．
        逐次推定におけるバイアス推定理論に基づく古典的手法 \cite{friedland1969}．
...
\section{おわりに}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
本研究では，遅延を含む観測環境において，位置誤差から速度バイアスを逆算して補正する
「遅延バイアスフィードバック法」を提案した．提案手法は単純リセット法と比較してRMSE
を約1/3 に低減し，計算負荷の高いカルマンフィルタと同等の精度を，軽量なアルゴリズムで実現した．
...
\begin{thebibliography}{9}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \bibitem{friedland1969}
  B. Friedland, "Treatment of Bias in Recursive Filtering," IEEE Transactions on Automatic Control, Vol. 14, No. 4, pp. 359--367, 1969.
